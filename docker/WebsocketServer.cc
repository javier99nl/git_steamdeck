                 ...
                 size_t _len)
{
  ...
  switch (_reason)
  {
    ...
    case LWS_CALLBACK_RECEIVE:
      {
        gzdbg << "LWS_CALLBACK_RECEIVE:" << (int)_len << ":" << self->rx_buf_idx << ":" << (int)lws_remaining_packet_payload(_wsi) << "/" << lws_is_final_fragment(_wsi) << std::endl;
	...
	if ( self->rx_buf_idx > 128 ) {
		gzerr << "Too large package (too many fragments) received, can't buffer for defragmentation\n";
                // Return non-zero to close the connection.
		self->rx_buf_idx = 0;
		return -1;
	}
	self->rx_buf[self->rx_buf_idx]=std::string((const char *)_in).substr(0, _len);
	self->rx_buf_idx++;

	//If lws_remaining_packet_payload == 0, call self->OnMessage, pointing to the buffer.
	if (lws_is_final_fragment(_wsi) || lws_remaining_packet_payload(_wsi) <= 0){
		std::ostringstream output;
		for (int i=0; i < self->rx_buf_idx; i++){
			output << self->rx_buf[i];
		}
		self->rx_buf_idx=0;
	        self->OnMessage(fd, output.str());
	}
        break;
      }
