...
void System::SaveKeyFrameImages(const string &filename)
{
    string path="/home/javi/datasets/keyframe";
    cout << endl << "Saving keyframe images to " << path << " ..." << endl;
    vector<Map*> vpMaps = mpAtlas->GetAllMaps();
    Map* pBiggerMap;
    int numMaxKFs = 0;
    for(Map* pMap :vpMaps)
    ...
    size_t size_vector= pBiggerMap->GetAllKeyFrames().size();
    vector<KeyFrame*> vpKFs = pBiggerMap->GetAllKeyFrames();	
    sort(vpKFs.begin(),vpKFs.end(),KeyFrame::lId);
    // Transform all keyframes so that the first keyframe is at the origin.
    // After a loop closure the first keyframe might not be at the origin.
    for(size_t i=0; i<size_vector; i++)
    {
        ...
        if(!pKF || pKF->isBad())
            continue;
        cv::Mat image= pKF->image;
        if(image.empty()==false){
	        string ext;
	        string number_images;
            number_images=to_string(i);
            path="/user_data/images/keyframe";
            path=path + number_images;
            ext=".jpg";
            path=path+ext;
