...
std::shared_ptr<rclcpp::Publisher<sensor_msgs::msg::PointCloud2>> map_points_pub;
std::shared_ptr<rclcpp::Publisher<sensor_msgs::msg::PointCloud2>> quality_points_pub;
image_transport::Publisher rendered_image_pub;

std::string map_frame_id="map";
std::string world_frame_id="world";
std::string pose_frame_id="pose";
...
tf2::Matrix3x3 tf_orb_to_ros(0, 0, -1, 1, 0, 0, 0, -1, 0); //The axes of 

void setup_ros_publishers(rclcpp::Node &node) {
  pose_pub = node.create_publisher<geometry_msgs::msg::PoseStamped>("/orbslam3/pose", 1);

  map_points_pub = node.create_publisher<sensor_msgs::msg::PointCloud2>("orbslam3/map_points", 1);
  quality_points_pub = node.create_publisher<sensor_msgs::msg::PointCloud2>("orbslam3/quality_points", 1);
...
void publish_ros_all_quality_mappoints(std::vector<ORB_SLAM3::MapPoint *> map_points,const rclcpp::Time &current_frame_time,std::shared_future<std_srvs::srv::Empty::Response::SharedPtr> future) 
{
  future.wait(); // wait for the service to finish
  //std::cout<<endl<<"RESET DONE!"<<endl;
  //std::this_thread::sleep_for(std::chrono::seconds(1));
  sensor_msgs::msg::PointCloud2 cloud = tracked_mappoints_to_qualitycloud(map_points, current_frame_time);
  //std::cout<<endl<<"Publishing all map points of size: "<<map_points.size()<<endl;
  quality_points_pub->publish(cloud);
}
void publish_ros_quality_mappoints(std::vector<ORB_SLAM3::MapPoint *> map_points,const rclcpp::Time &current_frame_time) 
{
  sensor_msgs::msg::PointCloud2 cloud = tracked_mappoints_to_qualitycloud(map_points, current_frame_time);
...
tf2::Transform from_orb_to_ros_tf_transform(cv::Mat transformation_mat) {
  ...
  return tf2::Transform(tf_orb_to_ros,tf2::Vector3(0,0,0))*(tf2::Transform(tf_camera_rotation, tf_camera_translation).inverse());
}
...
sensor_msgs::msg::PointCloud2
tracked_mappoints_to_qualitycloud(std::vector<ORB_SLAM3::MapPoint *> map_points,
                                rclcpp::Time current_frame_time) {
  ...
  cloud.is_bigendian = false;
  cloud.is_dense = true;
  cloud.point_step = 4 * sizeof(float) + sizeof(uint32_t);
  cloud.row_step = cloud.point_step * cloud.width;
  cloud.fields.resize(num_channels);
  ...
  for (int i = 0; i < 3; i++) {
      ...
      cloud.fields[3].offset = 3 * sizeof(float);
  ...
  for (unsigned int i = 0; i < cloud.width; i++) {
    if (map_points[i]) {
      ...
      uint n_obs =static_cast<uint>(map_points[i]->Observations());
      uint8_t r=n_obs;
      uint8_t g=0;
      uint8_t b=0;
        if(r<5){
        ...
        }else if(r>6 and r<11){
            r=255;
            g=255;
            b=0; 
        }else if(r>10 ){
          //std::cout<<std::endl<<"A point has more than 12 keyframes";
            r=0;
            g=255;
            b=0;
        }
      ...
      point_translation = tf_orb_to_ros * point_translation;
      float x= point_translation.x();
      float y= point_translation.y();
      float z= point_translation.z();
      uint8_t data_array[cloud.point_step];
      memcpy(data_array + 0 * sizeof(float), &x, sizeof(float)); // copy x as float 
      memcpy(data_array + 1 * sizeof(float), &y, sizeof(float)); // copy y as float 
      memcpy(data_array + 2 * sizeof(float), &z, sizeof(float)); // copy z as float
      memcpy(data_array + 3 * sizeof(float), &rgb, sizeof(uint32_t)); // copy rgb as uint32
      memcpy(cloud_data_ptr + (i * cloud.point_step), data_array, cloud.point_step);
    }
  }
